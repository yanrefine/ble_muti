...
/**
 * @brief Check if the device name in advertisement data matches TARGET_DEVICE_NAME
 *
 * @param advData Advertisement data
 * @param advLen Length of advertisement data
 * @param scanRspData Scan response data
 * @param scanRspLen Length of scan response data
 * @return uint8 TRUE if device name matches TARGET_DEVICE_NAME, FALSE otherwise
 */
static uint8 isTargetDeviceName(const uint8* advData, uint8 advLen,
                                  const uint8* scanRspData, uint8 scanRspLen)
{
    const uint8 targetLen = sizeof(TARGET_DEVICE_NAME) - 1; // exclude null terminator
    ...
    if (data && dataLen > 0) {
        ...
        while (idx < dataLen) {
            ...
            if (fieldLen == 0) break;
            if (idx + fieldLen > dataLen) break;

            uint8 adType = data[idx];
            if (adType == 0x09 || adType == 0x08) { // Complete or Shortened Local Name
                uint8 nameLen = fieldLen - 1;
                const uint8* nameData = &data[idx + 1];
                if (nameLen == targetLen && osal_memcmp(nameData, TARGET_DEVICE_NAME, targetLen) == 1) {
                    return TRUE;
                }
            }
            idx += fieldLen;
    ...
    // Then check scan response data (currently disabled as target device may not have scan response)
//    data = scanRspData;
//    dataLen = scanRspLen;
//    if (data && dataLen > 0) {
//        uint8 idx = 0;
